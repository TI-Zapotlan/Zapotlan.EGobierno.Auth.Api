...
namespace Zapotlan.EGobierno.Auth.Core.Services
{
    public class PersonaService : IPersonaService
    {
        ...
        public PagedList<Persona> Gets(PersonaQueryFilter filters)
        {
            ...
            if (filters.EstadoVida != null && filters.EstadoVida != Enumerations.PersonaEstadoVidaTipo.Ninguno)
            {
            ...
            {
                case Enumerations.PersonaOrdenFilterTipo.Nombre:
                    items = items.OrderBy(p => p.Nombres)
                        .ThenBy(p => p.PrimerApellido);
                    break;
                case Enumerations.PersonaOrdenFilterTipo.PrimerApellido:
                    items = items.OrderBy(p => p.PrimerApellido)
                        .ThenBy(p => p.SegundoApellido);
                    break;
                case Enumerations.PersonaOrdenFilterTipo.EstadoVida:
                    items = items.OrderBy(p => p.EstadoVida);
                    break;
                case Enumerations.PersonaOrdenFilterTipo.NombreDesc:
                    items = items.OrderByDescending(p => p.Nombres)
                        .ThenByDescending(p => p.PrimerApellido);
                    break;
                case Enumerations.PersonaOrdenFilterTipo.PrimerApellidoDesc:
